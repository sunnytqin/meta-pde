...
def solve_fenics(params, boundary_points=24, resolution=16):
    ...
    loss_fn_lib = importlib.import_module('.burgers_formulation.exact_2', package='src.burgers')
    ic_expression, bc_expression = loss_fn_lib.fa_expressions()
    ...
    for n in range(FLAGS.num_tsteps - 1):
        ...
        bc_left = fa.DirichletBC(V, bc_expr, left_wall)
        bc_right = fa.DirichletBC(V, bc_expr, right_wall)
        bc_horizontal = fa.DirichletBC(V, bc_expr, horizontal_walls)
