...
def res_nf_apply(
    ...
    for i, size in enumerate(sizes):
        ...
        if use_laaf:
            ...
            # omega0 in siren, hacked so we can choose to only do it on first layer
            x_hidden = nonlinearity(a * omega)
        else:
            x_hidden = nonlinearity(a)

        a = flax.nn.Dense(x_hidden, size, kernel_init=kernel_init)
        if use_laaf:
            x_hidden = laaf(a)
        elif use_nlaaf:
            x_hidden = nlaaf(a)
        ...
        res_a = flax.nn.Dense(x, size, kernel_init=kernel_init, bias=False)
        a = x_hidden + res_a
